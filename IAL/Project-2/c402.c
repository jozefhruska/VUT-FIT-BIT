...
**/
	...
	if ((* RootPtr) != NULL) {
		tBTNodePtr tempNode = (* RootPtr);

		while (true) {
			if (Content != tempNode->Cont) {
				if (tempNode->Cont > Content) {
					if (tempNode->LPtr != NULL) tempNode = tempNode->LPtr;
					else {
						if ((tempNode->LPtr = malloc(sizeof(struct tBTNode))) != NULL) {
							tempNode->LPtr->Cont = Content;
							tempNode->LPtr->LPtr = NULL;
							tempNode->LPtr->RPtr = NULL;
						}
						return;
					}
				} else {
					if (tempNode->RPtr != NULL) tempNode = tempNode->RPtr;
					else {
						if ((tempNode->RPtr = malloc(sizeof(struct tBTNode))) != NULL) {
							tempNode->RPtr->Cont = Content;
							tempNode->RPtr->LPtr = NULL;
							tempNode->RPtr->RPtr = NULL;
						}
						return;
					}
				}
...
**/
	...
	if (RootPtr != NULL) {
		...
		while (!SEmptyP(&stack)) Leftmost_Preorder(STopPopP(&stack)->RPtr, &stack);
	}
}
...
**/
	...
	if (RootPtr != NULL) {
		...
		while (!SEmptyP(&stack)) {
			tempNode = STopPopP(&stack);
			BTWorkOut(tempNode);
			Leftmost_Inorder(tempNode->RPtr, &stack);
		}
	}
}
...
**/
	...
	if (*RootPtr != NULL) {
		...
		while (!SEmptyP(&stack)) {
			...
			if (tempNode->LPtr != NULL) SPushP(&stack, tempNode->LPtr);
			if (tempNode->RPtr != NULL) SPushP(&stack, tempNode->RPtr);
