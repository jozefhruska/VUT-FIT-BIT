...
VertexIndex gpu_computeGLVertexID(
    ...
    return (indices != NULL ? indices[vertexShaderInvocation] : vertexShaderInvocation);
}
...
void const *gpu_computeVertexAttributeDataPointer(
  ...
  return (void const*) ((size_t) head->buffer + head->offset + ((size_t) gl_VertexID * (size_t) head->stride));
}
...
void gpu_runVertexPuller(GPUVertexPullerOutput *const              output,
  ...
  const GPUVertexPullerHead *pullerHead;

  for (size_t i = 0; i < MAX_ATTRIBUTES; i++) {
    pullerHead = &puller->heads[i];
    output->attributes[i] = pullerHead->enabled == 0 ? NULL : gpu_computeVertexAttributeDataPointer(pullerHead, vertexID);
  }
}
...
void gpu_runPrimitiveAssembly(
  ...
  for (size_t i = 0; i < nofPrimitiveVertices; i++) {
    gpu_runVertexPuller(&pullerOutput, puller, baseVertexShaderInvocation + i);
    shaderInput.gl_VertexID = gpu_computeGLVertexID(puller->indices, baseVertexShaderInvocation + i);
    shaderInput.attributes = &pullerOutput;
...
void gpu_computeScreenSpaceBarycentrics(
  ...
  float result = 1.0f / ((vertices[1].data[1] - vertices[2].data[1]) * (vertices[0].data[0] - vertices[2].data[0]) + (vertices[2].data[0] - vertices[1].data[0]) * (vertices[0].data[1] - vertices[2].data[1]));

  coords->data[0] = ((vertices[1].data[1] - vertices[2].data[1]) * (pixelCenter[0].data[0] - vertices[2].data[0]) + (vertices[2].data[0] - vertices[1].data[0]) * (pixelCenter[0].data[1] - vertices[2].data[1]));
  coords->data[1] = ((vertices[2].data[1] - vertices[0].data[1]) * (pixelCenter[0].data[0] - vertices[2].data[0]) + (vertices[0].data[0] - vertices[2].data[0]) * (pixelCenter[0].data[1] - vertices[2].data[1]));

  // Multiply by result
  coords->data[0] *= result;
  coords->data[1] *= result;
